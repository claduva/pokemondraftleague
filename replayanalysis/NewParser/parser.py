...
def move_function(line,parsedlogfile,results):
    ...
    #check if can miss
    if move in movesthatcanmiss.keys() and notimmune and targetnotprotected:
        try: 
            ...
            pass
    #check for miss
    if line[3].find("|[miss]")>-1 and notimmune and targetnotprotected:
    ...
    if move in increasedcrit and notimmune and targetnotprotected and target['pokemon'] not in ['Type: Null','Slowbro-Mega','Turtonator']:
        try:
            ...
            print(e)
    elif move in movesthatcancrit and notimmune and targetnotprotected and target['pokemon'] not in ['Type: Null','Slowbro-Mega','Turtonator']:
        try:
            if attacker['Focus Energy']:
    ...
    if move in moveswithsecondaryeffect.keys() and notimmune and targetnotprotected and (targetalive or move in selfboostmoves) and (movedfirst or move not in flinchmoves):
    #if move in moveswithsecondaryeffect.keys():# and notimmune and targetnotprotected and (targetalive or move in selfboostmoves) and (movedfirst or move not in flinchmoves):
