...
def secondary_check(attacker,target,move,line,results,parsedlogfile,attackingteam,notothermove):
    ...
    ['Ancient Power', ['boost', 'atk|1',attacker['nickname'], .1,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Charge Beam', ['boost', 'spa|1',attacker['nickname'], .7,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Steel Wing', ['boost', 'def|1',attacker['nickname'], .1,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Diamond Storm', ['boost', 'def|2',attacker['nickname'], .5,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Fiery Dance', ['boost', 'spa|1',attacker['nickname'], .5,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Silver Wind', ['boost', 'atk|1',attacker['nickname'], .1,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Meteor Mash', ['boost', 'atk|1',attacker['nickname'], .2,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    ['Ominous Wind', ['boost', 'atk|1',attacker['nickname'], .1,f"{attacker['pokemon']} received a stat boost as a secondary effect from {move}"]], 
    ...
    for line_ in turndata:
        if crittedteam=="p1a" and line_[2]=="move" and line_[3].split(":",1)[0]=="p2a" and line_[3].split("|")[2]==f"{crittedteam}: {crittedmon}":
            ...
            if attacker['pokemon'] not in ['Mareanie','Toxapex'] or (crittedmon_['psn']==None and crittedmon_['tox']==None):
                ...
                notothermove=True
                if line_[3].find("[from]Magic Bounce")>-1 or line_[3].find("[from]Copycat")>-1 or line_[3].find("[from]Metronome")>-1 or line_[3].find("[from]Magic Coat")>-1 or line_[3].find("[from]Nature Power")>-1 or line_[3].find("[from]Assist")>-1 or line_[3].find("[from]Me First")>-1:
                    notothermove=False
                if notothermove:
