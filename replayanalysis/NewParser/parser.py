...
def damage_function(line,parsedlogfile,results):
    ...
    if line[3].find("[from]")>-1:
        ...
        elif cause.find("move: Whirlpool")>-1 or cause.find("move: Infestation")>-1 or cause.find("move: Magma Storm")>-1 or cause.find("move: Wrap")>-1 or cause.find("move: Fire Spin")>-1 or cause.find("move: Thunder Cage")>-1 or cause.find("move: Bind")>-1 or cause.find("move: Sand Tomb")>-1 or cause.find("ability: Disguise|[of]")>-1 or cause.find("mimikyubusted")>-1 or cause.find("Mimikyu-Busted")>-1 or cause.find("G-Max Vine Lash")>-1:
            if team=="p1a":
...
def move_function(line,parsedlogfile,results):
    notothermove=True
    if line[3].find("[from]Magic Bounce")>-1 or line[3].find("[from]Copycat")>-1 or line[3].find("[from]Metronome")>-1 or line[3].find("[from]Magic Coat")>-1 or line[3].find("[from]Nature Power")>-1 or line[3].find("[from]Assist")>-1 or line[3].find("[from]Me First")>-1:
        notothermove=False
    #parse line
    move=line[3].split("|")[1]
    ...
    #check for hazards
    hazardmoves=["Stealth Rock","Spikes","Toxic Spikes","G-Max Steelsurge","G-Max Stonesurge"]
    move_=move
    if move =="G-Max Stonesurge":
        move_="Stealth Rock"
    if move in hazardmoves:
        if attackingteam=="p1a":
            results['team2'][move_]=attacker['nickname']
        elif attackingteam=="p2a":
            results['team1'][move_]=attacker['nickname']
    #moves that can miss
    ...
    if move.replace("Z-","") in ['Final Gambit','Healing Wish',"Lunar Dance","Memento","Explosion","Self-Destruct",'Misty Explosion']:
        move=move.replace("Z-","")
        #search for miss 
        turndata=list(filter(lambda x: x[1] == line[1] and x[0] > line[0] , parsedlogfile))[::-1]
        miss=False
...
def status_function(line,parsedlogfile,results):
    ...
    if line[3].find("[from] ability: ")>-1: 
        ...
        movesthatcausestatus=dict([
            ...
            ['brn',['Beak Blast','Will-O-Wisp','Fling','Psycho Shift','Burning Jealousy']],
