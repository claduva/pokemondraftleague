#luck: https://replay.pokemonshowdown.com/gen7anythinggoes-837812642
import requests
import json
...
def move_function(line,parsedlogfile,results):
    ...
    #check if can miss
    if move in movesthatcanmiss.keys() and notimmune:
        try: 
    ...
    if move in increasedcrit and notimmune and target['pokemon'] not in ['Type: Null','Slowbro-Mega','Turtonator']:
        try:
            ...
            pass
    elif move in movesthatcancrit and notimmune and target['pokemon'] not in ['Type: Null','Slowbro-Mega','Turtonator']:
        try:
            if attacker['Focus Energy']:
                attacker['luck']+=-50
                target['luck']+=50
    ...
    if move in moveswithsecondaryeffect.keys() and notimmune and targetalive:
        ...
        try:
            attacker['luck']+=-moveswithsecondaryeffect[move]*secondaryeffectmodifier
            target['luck']+=moveswithsecondaryeffect[move]*secondaryeffectmodifier
