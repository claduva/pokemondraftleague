...
def newreplayparse(replay):
    ...
    teams=['team1','team2']
    categories=['kills','deaths','luck','support','hphealed','damagedone']
    for team in teams:
...
def move_function(line,parsedlogfile,results):
    ...
    if move in movesthatcanmiss.keys():
        attacker['luck']+=(1-int(movesthatcanmiss[move])/100)
        target['luck']+=-(1-int(movesthatcanmiss[move])/100)
    #check for miss
    ...
    if move in moveswithsecondaryeffect.keys():
        attacker['luck']+=-(int(moveswithsecondaryeffect[move][1])/100)
        target['luck']+=(int(moveswithsecondaryeffect[move][1])/100)
